...
public class HeadUtil {
    ...
    public static String getSignature(Request request) {
        String accessToken = request.header(ConstantsHttp.Head_Token);
        if (!TextUtils.isEmpty(accessToken) && accessToken.length() >= 8) {
            ...
            Arrays.sort(signs);
            String digest = NetMD5.getMD5(signs[0] + signs[1]).toLowerCase();
            return digest;
        } else {
            return null;
        }
    ...
    public static String getUserAgent() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            try {
                userAgent = WebSettings.getDefaultUserAgent(AppBase.getApplication());
            } catch (Exception e) {
